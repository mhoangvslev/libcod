...
{
	LEVEL_SPACE;
	printf("setKeyInArray: int sub_807AB64(void *offsetData=%p, int key=%.8x, int a_plus_b_mod_fffd_plus_1=%.8x)\n", offsetData, key, a_plus_b_mod_fffd_plus_1);
	LEVEL_SPACE;
	printf("{\n");
	level++;
...
{
	LEVEL_SPACE;
	printf("int sub_807B064(void *offsetData=%p, int key=%.8x, int a_plus_b_mod_fffd_plus_1=%.8x)\n", offsetData, key, a_plus_b_mod_fffd_plus_1);
	LEVEL_SPACE;
	printf("{\n");
	level++;
	...
	*((int16_t*)v6 + 7) = a_plus_b_mod_fffd_plus_1;
	...
	LEVEL_SPACE;
	printf("v5=%.8x v5_2=%.8x v5_3=%.8x v5_4=%.8x\n", v5, v5_2, v5_3, v5_4);
	...
	LEVEL_SPACE;
	printf("v10=%.8x v10_2=%.8x v10_3=%.8x v10_4=%.8x v10_5=%.8x\n", v10, v10_2, v10_3, v10_4, v10_5);
...
{
	LEVEL_SPACE;
	printf("int sub_807B1E6(void *offsetData=%p, int key=%.8x)\n", offsetData, key);
	LEVEL_SPACE;
	printf("{\n");
	level++;
...
{
	LEVEL_SPACE;
	printf("int sub_807C71C(void *offsetData=%p, int key=%.8x)\n", offsetData, key);
	LEVEL_SPACE;
	printf("{\n");
	level++;
...
{
	LEVEL_SPACE;
	printf(/*AT*/ "int sub_807CB12(int v2=%.8x, aStackElement *stackElement=%p(type=%d))\n", v2, stackElement, stackElement->type);
	LEVEL_SPACE;
	printf("{\n");
	level++;
...
{
	LEVEL_SPACE;
	printf("int sub_80853B6(int key=%.8x)\n", key);
	LEVEL_SPACE;
	printf("{\n");
	level++;
...
int cdecl_injected_closer()
{
	...
	*/
	...
	{
	case 8:
		return gsc_graph_new();
	case 9:
		return gsc_graph_add_edge();
	case 10:
		return gsc_graph_add_vertex();
	case 11:
		return gsc_graph_delete();
	case 12:
		return gsc_graph_build();
	case 13:
		return gsc_graph_do_astar();
	//if (function == 14) { return gsc_tracefraction...;
	case 15:
		return gsc_graph_get_nearest_edge();
	...
	case 180:
		return gsc_mysql_test_0(); // simple connect, query, disconnect
	case 181:
		return gsc_mysql_test_1(); // same like _0, with stored procedures
#endif
	}
#endif
...
#define MAX_FILEHASH_SIZE   1024
	...
	*(int *)&BG_PlayAnim = 0x080D8F92;
	...
	case 1100:
		return stackPushInt(BG_PlayAnim(0x08705480, 0x00000118, 0x00000002, 0x00/*000567*/, 1, 0, 1)); // reload
	case 1101:
		return stackPushInt(BG_PlayAnim(0x08705480, 0x00000082, 0x00000003, 0x00/*000037*/, 1, /*0*/1, 1)); // strafejump
	case 1102:
		return stackPushInt(BG_PlayAnim(0x08705480, 0x00000083, 0x00000003, 0x00/*000037*/, 1, 0, 1)); // oO 1
	case 1103:
		return stackPushInt(BG_PlayAnim(0x08705480, 0x00000080, 0x00000003, 0x00/*000037*/, 1, 0, 1)); // oO 2
	case 1104:
		return stackPushInt(BG_PlayAnim(0x08705480, 0x00000000, 0x00000002, 0x00/*001000*/, 1, 1, 1)); // nothing
	case 1105:
		return stackPushInt(BG_PlayAnim(0x08705480, 0x00000085, 0x00000003, 0x00/*000600*/, 1, 1, 1)); // highjump
	case 1106:
		return stackPushInt(BG_PlayAnim(0x08705480, 0x00000081, 0x00000003, 0x00/*000037*/, 1, 1, 1)); // highjump
	case 1107:
		return stackPushInt(BG_PlayAnim(0x08705480,          6, 0x00000003, 0x00/*000037*/, 1, 1, 1));
	}
